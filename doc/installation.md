...
In development, we recommend you to use [venv](https://docs.python.org/3/tutorial/venv.html). It allows you to create a virtual environment: a self-contained directory tree that contains a Python installation for a particular version of Python, plus a number of additional packages. This approach avoids conflicts between the requirements of different applications in your computer. It also ensures that you don't overlook to add a new requirement to `setup.up by` using a library already installed in your computer.

To create an environment (in the parent directory, to avoid commit it by accident and that it mess up with the linters), run:
...
For more details about creating and using virtual environments, check [venv documentation](https://docs.python.org/3/tutorial/venv.html).
